...
| **Meshtastic firmware** (GPL-3.0) | Real LoRa mesh radio (IN865) | `ExternalNet` (out-of-process) | **Interop — done (over MQTT).** `lifeline-bridge::meshtastic` speaks genuine `ServiceEnvelope`/`MeshPacket` protobuf on a private app port, over MQTT (`mqtt` feature, `rumqttc`), so Lifeline rides real Meshtastic hardware + public brokers. GPL stays on-device — we only speak its wire format. Direct BLE/serial `LoRaInterface` remains a follow-up for radio-attached devices. Caps model `lora_in865`. |
