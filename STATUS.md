...
| **Meshtastic (MQTT) bearer** | ✅ | `lifeline-bridge::meshtastic` (`meshtastic`/`mqtt` features) — a second `ExternalNet` adapter over the seam: genuine Meshtastic `ServiceEnvelope`/`MeshPacket` protobuf (`prost`) carried on a private app port, so Lifeline rides **real LoRa mesh hardware + public Meshtastic MQTT brokers**. Sync `rumqttc` client → `MeshtasticNet` implements `ExternalNet` directly (no channel plumbing); proven end-to-end over a mock bus. Wired into the node behind the `meshtastic` feature: `LIFELINE_MESHTASTIC_MQTT=host:1883` |
